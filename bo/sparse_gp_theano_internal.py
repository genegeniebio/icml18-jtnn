...
def ratio(x):
    x = T.switch(T.lt(x, casting(-10)),
                 -(casting(1.0) / x - casting(1.0) /
                   x**3 + casting(3.0) / x**5 - casting(15.0) / x**7),
                 n_cdf(x) / n_pdf(x))
    return x
...
class Sparse_GP:
    ...
    def __init__(self, n_inducing_points, n_points, input_d, input_means,
        ...
        self.z = theano.shared(value=np.zeros((n_inducing_points,
                                               input_d)).astype(
            theano.config.floatX), name='z', borrow=True)
        self.lvar_noise = theano.shared(value=casting(
            0) * np.ones(1).astype(theano.config.floatX)[0], name='lvar_noise',
            borrow=True)
        ...
        if self.ignore_variances:
            ...
            if exact_output_vars:
                ...
                self.psi2 = compute_psi2(
                    self.lls, self.lsf, self.z, self.input_means,
                    self.input_vars)
    ...
    def get_params(self):
        ...
        return [self.lls, self.lsf, self.z, self.mParamPost, self.LParamPost,
                self.lvar_noise]
    ...
    ##
    # The next functions compute the log normalizer of each distribution
    # (needed for energy computation)
    #
    ...
    def getContributionToEnergy(self):
        ...
        # These methods sets the inducing points to be a random subset of the
        # inputs (we should receive more
    ...
    def initialize(self):
        ...
        lls = np.log(
            0.5 * (np.median(dist[np.triu_indices(input_means.shape[0], 1)]) +
                   1e-3)) * np.ones(input_means.shape[1])
        ...
        # We initialize the cavity and the posterior approximation to the prior
        # but with a small random
        # mean so that the outputs are not equal to zero (otherwise the output
        # of the gp will be zero and
    ...
    # This sets the node for prediction. It basically switches the cavity
    # distribution to be the posterior approximation
    ...
    def compute_log_averaged_ei(self, x, X, randomness, incumbent):
        ...
        Kzz_expanded = compute_kernel(self.lls, self.lsf, z_expanded,
                                      z_expanded) + T.eye(
