...
def ratio(x):
    x = T.switch(T.lt(x, casting(-10)), -(casting(1.0) / x - casting(1.0) /
                                          x**3 + casting(3.0) / x**5 - casting(15.0) / x**7), n_cdf(x) / n_pdf(x))
    return x
...
class Sparse_GP:
    ...
    # n_points are the total number of training points (that is used for
    # cavity computation)
    ...
    def __init__(self, n_inducing_points, n_points, input_d, input_means, input_vars, training_targets):
        ...
        self.LParamPost = theano.shared(value=initial_value.astype(
            theano.config.floatX), name='LParamPost', borrow=True)
        self.mParamPost = theano.shared(value=initial_value[:, 0: 1].astype(
            theano.config.floatX), name='mParamPost', borrow=True)
        self.lls = theano.shared(value=np.zeros(input_d).astype(
            theano.config.floatX), name='lls', borrow=True)
        self.lsf = theano.shared(value=np.zeros(1).astype(
            theano.config.floatX)[0], name='lsf', borrow=True)
        self.z = theano.shared(value=np.zeros((n_inducing_points, input_d)).astype(
            theano.config.floatX), name='z', borrow=True)
        self.lvar_noise = theano.shared(value=casting(
            0) * np.ones(1).astype(theano.config.floatX)[0], name='lvar_noise', borrow=True)
    ...
    def compute_output(self):
        ...
        self.meanCavity = T.dot(self.covCavity, casting(
            self.n_points - self.set_for_training) / casting(self.n_points) * self.mParamPost)
        ...
        v_out = T.exp(self.lsf) + T.dot(self.Kxz *
                                        T.dot(self.Kxz, self.B), T.ones_like(self.z[:, 0: 1]))
        ...
        if self.ignore_variances:
            ...
            self.EKxz = compute_psi1(
                self.lls, self.lsf, self.input_means, self.input_vars, self.z)
            ...
            if exact_output_vars:
                ...
                self.psi2 = compute_psi2(
                    self.lls, self.lsf, self.z, self.input_means, self.input_vars)
                ll = T.transpose(self.EKxz[:, None, :]
                                 * self.EKxz[:, :, None], [1, 2, 0])
                kk = T.transpose(self.Kxz[:, None, :]
                                 * self.Kxz[:, :, None], [1, 2, 0])
                v1 = T.transpose(T.sum(T.sum(T.shape_padaxis(
                    self.B2, 2) * (self.psi2 - ll), 0), 0, keepdims=True))
                v2 = T.transpose(T.sum(T.sum(T.shape_padaxis(
                    self.B, 2) * (self.psi2 - kk), 0), 0, keepdims=True))
                ...
                for j in range(1, n + 1):
                    ...
                    self.kplus = T.sqrt(casting(1.0) / casting(2 * n)) * compute_kernel(
                        self.lls, self.lsf, self.input_means + inc, self.z)
                    self.kminus = T.sqrt(casting(1.0) / casting(2 * n)) * compute_kernel(
                        self.lls, self.lsf, self.input_means - inc, self.z)

                    v1 += T.dot(self.kplus * T.dot(self.kplus,
                                                   self.B2), T.ones_like(self.z[:, 0: 1]))
                    v1 += T.dot(self.kminus * T.dot(self.kminus,
                                                    self.B2), T.ones_like(self.z[:, 0: 1]))
                    v2 += T.dot(self.kplus * T.dot(self.kplus, self.B),
                                T.ones_like(self.z[:, 0: 1]))
                    v2 += T.dot(self.kminus * T.dot(self.kminus,
                                                    self.B), T.ones_like(self.z[:, 0: 1]))
    ...
    def getLogNormalizerPosterior(self):
        ...
        return casting(0.5 * self.n_inducing_points * np.log(2 * np.pi)) + casting(0.5) * T.nlinalg.LogDetPSD()(self.covPosterior) + \
            casting(0.5) * T.dot(T.dot(T.transpose(self.meanPosterior),
                                       self.covPosteriorInv), self.meanPosterior)
    ...
    def getContributionToEnergy(self):
        ...
        # inputs than inducing points), the length scales are set to the mean
    ...
    def initialize(self):
        ...
        selected_points = np.random.choice(
            input_means.shape[0], self.n_inducing_points, replace=False)
        z = input_means[selected_points, :]

        # If we are not in the first layer, we initialize the length scales to
        # one
        ...
        lls = np.log(
            0.5 * (np.median(dist[np.triu_indices(input_means.shape[0], 1)]) + 1e-3)) * np.ones(input_means.shape[1])
        ...
        # If we are not in the first layer, we reduce the variance of the L and
        # m

        L = np.random.normal(size=(self.n_inducing_points,
                                   self.n_inducing_points)) * 1.0
    ...
    def compute_log_ei(self, x, incumbent):
        ...
        meanCavity = T.dot(covCavity, casting(
            self.n_points - self.set_for_training) / casting(self.n_points) * self.mParamPost)
        ...
        v_out = T.exp(self.lsf) + T.dot(Kxz * T.dot(Kxz, B),
                                        T.ones_like(self.z[:, 0: 1]))  # + T.exp(self.lvar_noise)
    ...
    def compute_log_averaged_ei(self, x, X, randomness, incumbent):
        ...
        KXX = compute_kernel(self.lls, self.lsf, X, X) - T.dot(T.dot(KXz, KzzInv),
        ...
        KxX = KxX - T.dot(T.dot(KxXz[0: x.shape[0], :],
                                KzzInv), KxXz[x.shape[0]: xX.shape[0], :].T)
        ...
        new_predictive_mean = T.tile(
            m_old_x, [1, randomness.shape[1]]) + T.dot(KxX, T.dot(KXXInv, samples_internal))
        ...
        v_out = T.exp(self.lsf) - T.dot(Kxz_expanded * T.dot(Kxz_expanded,
                                                             Kzz_expandedInv), T.ones_like(z_expanded[:, 0: 1]))
        new_predictive_var = T.tile(v_out, [1, randomness.shape[1]])

        s = (incumbent - new_predictive_mean) / T.sqrt(new_predictive_var)

        log_ei = T.log((incumbent - new_predictive_mean) *
                       ratio(s) + T.sqrt(new_predictive_var)) + log_n_pdf(s)
