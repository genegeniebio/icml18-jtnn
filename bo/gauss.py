...
def compute_kernel_numpy(lls, lsf, x, z):
    ...
    r2 = np.outer(np.sum(x * x / lsre, 1), np.ones(z.shape[0])) - 2 * np.dot(
        x / lsre, z.T) + np.dot(1.0 / lsre, z.T ** 2)
...
def compute_psi1(lls, lsf, xmean, xvar, z):
    ...
    psi1 = sf * \
        T.outer(constterm2, T.ones_like(
            z[:, 0: 1])) * T.exp(-np.float32(0.5) * r2_psi1)
